...
_GAME_TYPE = pyspiel.GameType(
    short_name="python_simple_poker",
    long_name="Python Simple Poker",
...
class SimplePokerGame(pyspiel.Game):
  """A Python version of Simple poker."""
  ...
  def new_initial_state(self):
    ...
    return SimplePokerState(self)

  def make_py_observer(self, iig_obs_type=None, params=None):
    """Returns an object used for observing game state."""
    return SimplePokerObserver(
        iig_obs_type or pyspiel.IIGObservationType(perfect_recall=False),
        params)


class SimplePokerState(pyspiel.State):
  """A python version of the Simple poker state."""
  ...
  def __init__(self, game):
    ...
    self.cards = []
    self.bets = []
    self.pot = [1.0, 1.0] #How much each player has put into the total pot
  ...
  def chance_outcomes(self):
    ...
    assert self.is_chance_node() #PARENT METHOD -> checks if playerID == chanceID
    if self.cards:
...
pyspiel.register_game(_GAME_TYPE, SimplePokerGame)
